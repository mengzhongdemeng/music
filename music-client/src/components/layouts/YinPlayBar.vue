<template>
  <div class='play-bar' :class='{show:!toggle}'>
    ...
    <div class='kongjian'>
      ...
      <!--播放模式-->
      <div class='item item-mode' @click='playMode'>
        <!--顺序播放-->
        <div v-show='modeId == 1' class='iconfont icon-shunxubofang'></div>
        <!--循环播放-->
        <div v-show='modeId == 2' class='iconfont icon-danquxunhuan'></div>
        <!--随机播放-->
        <div v-show='modeId == 3 || modeId == 0' class='iconfont icon-random'></div>
      </div>
      <!--音量-->
...
export default {
  ...
  watch: {
    ...
    autoNext () {
      ...
      // 循环播放
      if (this.modeId === 2) {
        this.$store.commit('setCurrentPlayIndex', this.currentPlayIndex)
        this.toPlay(this.currentPlayList[this.currentPlayIndex].url)
        this.$store.commit('setIsPlay', true)
      }
      // 随机播放
      if (this.modeId === 3 || this.modeId === 0) {
        this.shuffle(this.currentPlayList)
        this.next()
      }
    }
  },
  ...
  methods: {
    ...
    shuffle (source) {
      ...
      this.$store.commit('setCurrentPlayList', arr)
    },
    // 改变播放模式
    playMode () {
      this.modeId++
        ...
        HttpManager.setCollection(params)
          .then(res => {
            if (res.code === 1) {
              ...
              this.$store.commit('setIsCollection', false)
              params.append('userId', this.userId)
              params.append('type', 0) // 0 代表歌曲， 1 代表歌单
              params.append('songId', this.songId)
              HttpManager.deleteCollection(params)
              this.$notify({
                title: '取消收藏',
                type: 'success'
              })
